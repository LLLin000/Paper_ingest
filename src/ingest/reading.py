...
def count_vision_fallback_pages(vision_dir: Path) -> int:
...
def apply_pipeline_fallback_degradation(
    summary_status: dict[str, Any],
    reading_fallback_count: int,
    vision_fallback_pages: int,
) -> dict[str, Any]:
    updated = dict(summary_status)
    reason_codes_raw = updated.get("reason_codes", [])
    if isinstance(reason_codes_raw, list):
        reason_codes = [str(code).strip() for code in reason_codes_raw if str(code).strip()]
    else:
        reason_codes = []

    if reading_fallback_count > 0:
        reason_codes.append("reading_model_fallback_used")
    if vision_fallback_pages > 0:
        reason_codes.append("vision_fallback_used")

    dedup_reason_codes = list(dict.fromkeys(reason_codes))
    updated["reason_codes"] = dedup_reason_codes
    updated["status"] = "degraded" if dedup_reason_codes else "full"
    return updated
...
def run_reading(
    ...
    summary_status = apply_pipeline_fallback_degradation(
        summary_status,
        reading_fallback_count=len(fault_events),
        vision_fallback_pages=count_vision_fallback_pages(run_dir / "vision"),
    )
